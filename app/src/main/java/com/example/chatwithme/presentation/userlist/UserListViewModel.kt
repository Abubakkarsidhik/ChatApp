...
class UserListViewModel @Inject constructor(
    ...
    private fun loadAcceptFriendRequestListFromFirebase() {
        viewModelScope.launch {
            println("loadAcceptedFriendRequestListFromFirebase called")
            userListScreenUseCases.loadAcceptedFriendRequestListFromFirebase.invoke()
                .collect { response ->
                    when (response) {
                        is Response.Loading -> {}
                        is Response.Success -> {
                            println("loadAcceptedFriendRequestListFromFirebase ${response.data}")
    ...
    private fun loadPendingFriendRequestListFromFirebase() {
        viewModelScope.launch {
            println("loadPendingFriendRequestListFromFirebase called")
            userListScreenUseCases.loadPendingFriendRequestListFromFirebase.invoke()
                .collect { response ->
                    when (response) {
                        is Response.Loading -> {}
                        is Response.Success -> {
                            println("loadPendingFriendRequestListFromFirebase ${response.data}")
                ...
                when (response) {
                    ...
                    is Response.Success -> {
                        if (response.data.equals(FriendListRegister())) {
                            ...
                            println("checkFriendListRegisterIsExistFromFirebase $chatRoomUUID,$acceptorEmail," +
                                    "$acceptorUUID,$acceptorOneSignalUserId")
