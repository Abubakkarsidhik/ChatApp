...
class ProfileViewModel @Inject constructor(
        ...
        viewModelScope.launch {
            useCases.uploadPictureToFirebase(uri).collect { response ->
                when(response){
                    ...
                    is Response.Success -> {
                        println("picture response -- $response")
                        //Picture Uploaded
    ...
    fun updateProfileToFirebase(myUser: User) {
        viewModelScope.launch {
            println("updateProfileToFirebase called")
            useCases.createOrUpdateProfileToFirebase(myUser).collect { response ->
                when(response){
                    ...
                    is Response.Success -> {
                        println("updateProfileToFirebase ${response}")
                        isLoading.value = false
                        if(response.data){
    ...
    private fun loadProfileFromFirebase() {
        println("loadProfileFromFirebase called")
        viewModelScope.launch {
            useCases.loadProfileFromFirebase().collect { response ->
                when(response){
                    is Response.Loading -> {
                        println("loadProfileFromFirebase loading ${response}")
                        isLoading.value = true
                    }
                    is Response.Success -> {
                        println("loadProfileFromFirebase success $response")
                        println("loadProfileFromFirebase $response")
                        userDataStateFromFirebase.value = response.data
                        delay(500)
                        isLoading.value = false
                    }
                    is Response.Error -> {
                        println("loadProfileFromFirebase error $response")
                    }
                }
            }
        }
